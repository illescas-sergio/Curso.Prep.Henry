...
function dePalabrasAFrase(palabras) {
  ...
  for(let i = 0; i <= palabras.length-1; i++)
        stringNuevo.push(palabras[i]) + stringNuevo.push(espacio);
             stringNuevo.pop();
        for(let j = 0; j <= stringNuevo.length-1; j++)
        stringSalida= stringSalida + stringNuevo[j];
...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  let cantidadArgumentos = arguments.length;
        if(cantidadArgumentos === 0){
            return cantidadArgumentos
        } else if(cantidadArgumentos === 1){
            return arguments[0]
        } 
    let producto = 1;
